<template>
	<div class="container">
		<div class="flex">
			<m-header @backClick="handleClose" headerTitle="添加联系人"></m-header>
		</div>
		<div class="main">
			<form ref="parent" action="" name="form">
				<ul class="from-table">
					<li class="table-li" >
						...
						<input name="role" class="value-content" type="text" readonly v-model="roleNameSelect" @click="selectRole" placeholder="请选择">
						<img class="back" src="../../assets/images/list-arrow.png" alt="">
					</li>
					<li class="table-li">
						<img class="required" src="../../assets/images/list-asterisk.png" alt="">
						<span class=" key-content">姓名</span>
						<input name="roleName" class="value-content" type="text" maxlength="20" v-model="roleName" placeholder="请输入">
					</li>
					<li class="table-li">
						<span class="key-content no-require">手机号码</span>
						<input name="rolePhone" class="value-content" type="text" maxlength="11" v-model="rolePhone" placeholder="请输入">
					</li>
					<li class="table-li">
						<span class="key-content no-require">微信</span>
						<input class="value-content" type="text" maxlength="20" v-model="roleWx" placeholder="请输入">
					</li>
...
export default {
	...
	methods: {
		...
		sureRole (titleText, value, index) {
			this.roleNameSelect = value[index].description
		...
		submitFrom () {
			if (this.submitFlag && this.validate()) {
				...
				formData.append('contactsDTO.userName', this.roleName.trim())
				formData.append('contactsDTO.phone', this.rolePhone)
				formData.append('contactsDTO.wechat', this.roleWx.trim())
				formData.append('contactsDTO.fixedFlag', 0)
				formData.append('contactsDTO.remark', this.remark)
				formData.append('contactsDTO.creatorId', this.infoInform.id)
