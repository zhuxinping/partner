<template>
	<div class="active">
		<div class="spr-title">
			...
			<ul class="tab">
				<li v-for="(item, index) in tabs" :key="index"  @click="ChangeTab(index)" :class="{'active':current == index}">{{item}}</li>
			</ul>
		</div>
		<div class="tab-db">
	...
	methods: {
		...
		ChangeTab (index) {
			...
			if (index === 1) {
				...
				this.chartSettings.metrics = ['当日活跃学生率', '当日活跃家长率', '当日活跃园丁率']
			...
			if (this.listDate.list) {
				this.listDate.list.forEach((item, i) => {
					...
					if (index === 1) {
						object = {
							'日期': item.day + '日',
							'当日活跃学生率': parseFloat(item.currentActiveStudentRatioStr) / 100,
							'当日活跃家长率': parseFloat(item.currentActiveParentsRatioStr) / 100,
							'当日活跃园丁率': parseFloat(item.currentActiveTeacherRatioStr) / 100
						}
					} else {
						object = {
							'日期': item.day + '日',
							'当日活跃学生数': item.activeStudent,
							'当日活跃家长数': item.activeParent,
							'当日活跃园丁数': item.activeTeacher
						}
					}
